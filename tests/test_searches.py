...
from src.searches import count_categories, search_string_in_transactions
...
def test_search_string_in_transactions(input_operations):
    assert search_string_in_transactions(input_operations, "счет") == [
        {
            "id": 2177828.0,
            "state": "EXECUTED",
            "date": "2022-04-14T15:14:21Z",
...
def test_count_categories(input_operations):
    assert count_categories(input_operations, ["Перевод с карты на карту", "Перевод со счета на счет"]) == {
